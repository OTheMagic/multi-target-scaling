...
coordinate-wise interval length, prediction-set volume, and computational
cost. Independent-noise benchmarks establish the effect of unequal outcome
scales; dependent and partially heteroskedastic models examine how this
behavior changes with cross-output and input-dependent variation. We then
consider quantile-regression scores and six real datasets.
Appendix~\ref{app:additional-experiments} provides distribution-specific
comparisons, sensitivity analyses, heavy-tail and contamination stress tests,
and comparisons with the population oracle, the unenclosed local union, and
a convex shape-template method.
...
and the target coverage is $1-\alpha=0.9$. All methods in a comparison share
the fitted regression model and the calibration and test observations.
...
\[
  ...
  \qquad j\in[10],
\]
where every $f_j$ is linear and its coefficients are sampled once
from $\operatorname{Unif}(-10,10)$.

\paragraph{Independent noise and coordinate heterogeneity.}
Under independent Gaussian noise with scales $(10,9,\ldots,1)$, TSCP
combines near-nominal joint coverage with smaller prediction-set volumes
than Point CHR and Unscaled Max. With unit noise scale in every coordinate,
Unscaled Max is the most efficient, while TSCP remains competitive.
Figures~\ref{fig:gaussian-heter} and~\ref{fig:gaussian-homo}, together with
the Laplace, mixture, and Gamma comparisons in
Appendix~\ref{app:simulations}, show that the benefit of standardization
depends on the distribution of variation across outcomes. These
independent-noise benchmarks use randomized splits of a fixed simulated
training/test pool, as specified in Appendix~\ref{app:experimental-protocols}.

\paragraph{Heterogeneous and dependent noise.}
To study dependence and input-dependent variation with independently
replicated datasets, each repetition draws 7,200 observations to fit ordinary
least squares, 800 test observations, and a separate calibration sample of
size $n\in\{30,50,100,300,500\}$. The dependent-noise experiment uses
