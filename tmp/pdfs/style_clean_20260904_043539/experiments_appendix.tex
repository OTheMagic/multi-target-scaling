...
\subsubsection{Coverage and Volume}
Figure~\ref{fig:app-heavy-tails} compares calibration sizes $n=30$ and
$n=500$ across the three degrees of freedom.
...
\FloatBarrier
\subsubsection{Additional Comparison with Point CHR}
\label{app:hevy_tailed_simulation}
...
$n\in\{30,500\}$. Results average 200
...
larger size. As in Figure~\ref{fig:app-heavy-tails}, the
...
\begin{figure}[!htbp]
    ...
    over 30 repetitions. Panels report mean joint coverage, residual-space
    volume, and construction time in milliseconds. Volume and runtime use
    logarithmic axes. The dashed line marks target coverage $0.9$;
    downward triangles identify coverage means below the displayed lower
    limit $0.6$. TSCP-LWC is evaluated only at $d\in\{2,3,4\}$ because of
...
We quantify uncertainty for 58 TSCP settings from the dependence,
heteroskedasticity, CQR, sensitivity, contamination, and shape-template
comparisons, the heavy-tail comparison in Figure~\ref{fig:app-heavy-tails},
and the real-data benchmark in Table~\ref{tab:real-data}. This calculation
excludes the distribution tables in
Appendix~\ref{app:additional-distribution-experiments}, the oracle and
dimension-scaling comparisons in Appendix~\ref{app:ours_simulation},
Figure~\ref{fig:t}, and the separate real-data coordinate-analysis batch.
Within these 58 settings, the corresponding
...
For the synthetic experiments included in this uncertainty analysis, training, test, and calibration
