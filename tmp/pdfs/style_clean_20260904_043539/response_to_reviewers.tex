...
\item \textbf{Integration of earlier evidence.} The independent Gaussian
comparisons now lead \mssec{sec:absolute-residual-experiments}, followed by
dependence and partial heteroskedasticity. Earlier distribution, oracle,
local-union, runtime, and real-data results are integrated by topic in
\msapp{app:additional-experiments}. Redrawn plots use consistent method names,
colors, and formatting; numerical values are preserved.
...
See \path{unresolved_reviewer_concerns.md} for remaining actions.
\action{Reconcile the archived CSVs with the declared fresh-sampling design:
the saved notebook generates training/test data outside the repetition loop.
Restyling does not verify sampling history; see \path{sampling_provenance.md}.}
