    ...
    for alpha in alpha_list:
        for index_dim, dim in enumerate(dim_list):
            for index_sample, sample in enumerate(sample_list):
                ...
                avg = trials
                if method == "lpr":
                    outputs.append([alpha, dim, sample, c_vol_lpr / avg, c_cal_lpr / avg, c_test_lpr / avg, runtime_lpr / avg])
                elif method == "lpr_o":
                    outputs.append([alpha, dim, sample, c_vol_lpr_o / avg, c_cal_lpr_o / avg, c_test_lpr_o / avg, runtime_lpr_o / avg])
                elif method == "ds":
                    outputs.append([alpha, dim, sample, c_vol_ds / avg, c_cal_ds / avg, c_test_ds / avg, runtime_ds / avg])
                elif method == "npr":
                    outputs.append([alpha, dim, sample, c_vol_npr / avg, c_cal_npr / avg, c_test_npr / avg, runtime_npr / avg])

    # Return results as DataFrame
    columns = ["alpha", "n_dim", "n_scores", "coverage_vol", "coverage_cal", "coverage_test", "runtime"]
