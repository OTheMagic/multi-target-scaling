...
def data_spliting_CHR_prediction(scores, 
    ...
    # Recalibration uses its own sample size. With an odd total, the
    # second half is larger and can require a higher conformal rank.
    adjustment_rank = conformal_rank(len(excess_scores), alpha)
    if adjustment_rank <= len(excess_scores):
        adj1 = np.partition(excess_scores, adjustment_rank - 1)[adjustment_rank - 1]
    else:
        adj1 = np.inf
