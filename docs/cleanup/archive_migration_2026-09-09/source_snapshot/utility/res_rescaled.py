...
def mean_index_solver(scores):
    ...
    # Use the last tied value below the mean, not argmax's first occurrence.
    # Otherwise a mass at zero can select [0, 0] instead of the mean cell.
    return np.array([
        np.searchsorted(scores_sorted[:, j], scores_mean[j], side="right")
