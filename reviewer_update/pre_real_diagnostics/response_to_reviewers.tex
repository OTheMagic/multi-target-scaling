...
\item \textbf{Preservation of earlier evidence.} All historical Gaussian plots,
distribution tables, oracle/local-union comparisons, dimension-scaling timings,
heavy-tail plots, and the original real-data table are retained in
\msapp{app:legacy-experiments}. Their original resampling protocol is
distinguished from the new fresh-draw experiments.
...
not edited during integration.
\end{itemize}
\textbf{Reviewer audit:} 8 addressed, 6 partially addressed, and 6 open.
...
\label{response:E1}
We thank the Editor for the opportunity to revise. The integrated manuscript now includes the requested coordinate-length plots, CQHR and shape-template comparisons, partial heteroskedasticity, contamination stress tests, and main-text runtime evidence. Every previously included experiment is retained: older comparisons are organized in \msapp{app:legacy-experiments}, while the original two-point energy example remains in the body.

This letter distinguishes completed work from remaining actions. Of the twenty reviewer points, eight are addressed, six partly addressed, and six open. We do not claim that the revision is submission-ready until the outstanding diagnostics and non-experimental corrections listed below and in \path{unresolved_reviewer_concerns.md} have been considered.
...
\action{Section 3.4.1 currently infers conditional coverage given a working hypothesis from a marginal oracle guarantee. That does not follow from the stated containment alone. Correct this using the pointwise oracle-acceptance argument, and review the related large-sample tightness language. The exposition has improved, but this point cannot be marked fully resolved while that claim remains.}
...
The timings exclude the shared regression fit and residual computation. Real-data values are retained measurements from the original runs, not a new large-scale benchmark. The theoretical range remains $O(d^2n\log n)$ to $O(d^2n^2)$, with thresholds reusable across test inputs. These measurements demonstrate observed cost at the tested sizes, not improved worst-case complexity. Backward-search frequency is a separate unresolved question in R1.Q2.
...
\point{R1.Q2. Frequency and cost of backward search}{Open}{How often does the algorithm fall into the less favorable
...
Overall runtimes are now visible, but the saved real-data summaries contain no branch indicators. They cannot establish backward-search frequency, and a small mean runtime is not evidence that this branch was rare.

Backward search differs from empty-mean-cell fallback: an empty cell returns GWC, whereas a nonempty cell can still require backward rather than binary search in some coordinates. For each dataset, useful diagnostics are the fraction of runs with any backward search, the fraction of coordinate searches that are backward among non-fallback runs, and construction times conditional on the branch. Each denominator must be stated.

\action{Collect the diagnostics for the six datasets, ideally by replaying the original 200 splits, or explicitly identify new diagnostic runs. Insert the measured counts, conditional runtimes, and table location here. This is an unanswered empirical request, not simply a missing citation.}
...
\point{R2.M2. Dimension-specific comparisons}{Partially addressed}{After Theorem 1 you discuss that the overall coverage cannot be used
...
The original energy example is retained in \mstab{tab:toy}, but intervals for two test observations are not an across-split real-data coordinate analysis. Joint validity also does not imply equal marginal coverage.

\action{Add real-data coordinate-wise mean lengths or marginal coverages with uncertainty and a stated split protocol, or explain the omission. The saved aggregate real-data tables cannot reconstruct these statistics.}

\point{R2.M3. Frequency of failure of the regularity condition}{Open}{In section 3.4.4 it is stated: ``Under the regularity condition
...
Section 3.4.4 explains the GWC fallback and now correctly identifies the exceptional empty-cell condition. However, it still says that the regularity condition typically holds without supplying the requested frequency.

The real-data files contain coverage, volume, and mean runtime, not fallback indicators. The appropriate unit is one calibration split, not each test observation, because thresholds are reused. Equality between local and global returned rectangles is not proof of a fallback event.

\action{Instrument the actual empty-cell condition and report its count out of 200 splits for every dataset, with percentages and a table reference. Keep fallback frequency separate from R1.Q2's backward-search frequency. Do not replace unmeasured counts with zero.}
...
\label{response:R2.m1}
Joint coverage need not equal either coordinate's marginal coverage, and the two marginals need not match. However, \msfig{fig:joint-prediction} remains the original schematic: its rightmost panel labels joint coverage only.

\action{Add both coordinate marginal labels to the figure and drawing source. If values such as $92\%$ and $93\%$ are chosen illustratively, identify them as schematic rather than computed from the unchanged plot. This non-experimental figure has not been edited.}

\point{R2.m2. Meaning of ``uniformly tight'' in the population oracle}{Open}{In section 2.5 when discussing the oracle prediction set your
...
Section 2.5 still uses \emph{uniformly tight} with the same rationale, and related language remains in Section 2.4.

\action{Define or replace the term: specify whether it means width, volume, oracle approximation, or marginal coverage. Retain a balance claim only with the requisite assumptions and justification. The new bar plots illustrate width allocation, not equal marginal coverage.}

\point{R2.m3. Labeling the boundaries in Figure 2}{Open}{In section 3.4.4, after you introduce $L_j$, you discuss what
...
\label{response:R2.m3}
\msfig{fig:global-to-local} still lacks endpoint labels. The revised method denotes the residual boundaries by $\mathcal L_1,\mathcal L_2$, and the figure should use that convention rather than the old $L_j$ notation.

\action{Add margin labels or guide lines for $\mathcal L_1,\mathcal L_2$ to the schematic and its drawing source, identifying residual-space axes. For absolute scores, full outcome lengths are $2\mathcal L_j$. This protected methodology figure has not been changed.}
